...
export default function ContactFormModal({ onClose }: { onClose: () => void }) {
  ...
  return (
    <motion.div
      ...
      className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm overflow-y-auto px-3 py-5 sm:px-5"
    >
      {showThankYou && <ThankYouModal open={showThankYou} onClose={onClose} />}
      <motion.div
        initial={{ x: 80, opacity: 0 }}
        animate={{ x: 0, opacity: 1 }}
        exit={{ x: 80, opacity: 0 }}
        transition={{ delay: 0.1, type: "spring", stiffness: 80, damping: 18 }}
        className="relative w-full max-w-4xl mx-auto bg-[#18181b] rounded-[1.6rem] sm:rounded-[2rem] shadow-2xl flex flex-col md:flex-row px-4 py-8 sm:p-8 md:p-12 lg:p-16 gap-7 md:gap-8 border border-zinc-700/70"
