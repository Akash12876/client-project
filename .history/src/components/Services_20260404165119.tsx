      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 w-full max-w-7xl">
        {services.map((service, idx) => {
          ...
          return (
            <motion.div
              key={service.title}
              className="relative glassmorphic-card border border-purple-400/40 rounded-t-[3.5rem] rounded-b-3xl shadow-2xl flex flex-col justify-between p-7 md:p-8 min-h-[340px] group overflow-hidden transition-transform duration-300 backdrop-blur-xl items-start text-left"
            ...
            >
              ...
              <h3 className={`text-xl md:text-2xl font-bold mb-2 drop-shadow-lg self-start ${colorMap[idx]}`}>{service.title}</h3>
              <p className="text-white/80 mb-6 text-base md:text-lg min-h-[72px] self-start">{service.desc}</p>
              <motion.button
                className="mt-auto bg-gradient-to-r from-purple-500 via-indigo-500 to-pink-500 text-white font-bold px-7 py-2.5 rounded-full shadow-lg border-2 border-white/10 relative overflow-hidden group/button transition-all duration-300 hover:scale-105 hover:from-pink-500 hover:to-purple-500 self-start"
