...
const cardVariants: Variants = {
  hidden: (idx: number) => {
    ...
    return {
      ...
      scale: 0.94,
      filter: "blur(14px)",
      clipPath: "inset(0 0 28% 0 round 2rem)",
    };
  },
  show: (idx: number) => ({
    opacity: 1,
...
const panelVariants: Variants = {
  hidden: { scaleY: 1, opacity: 1, originY: 1 },
  show: (idx: number) => ({
    scaleY: 0,
    opacity: 0,
    transition: {
  ...
  return (
    ...
    >
      <m.div
        ...
        style={{
          background: `linear-gradient(180deg, rgba(0,0,0,0.96) 0%, ${service.accent}22 100%)`,
        }}
      />
      ...
      <div className="relative z-10 flex h-full flex-col">
        <m.div variants={contentVariants} custom={idx} className="mb-7 flex items-start justify-between gap-4">
          <div>
        ...
        <m.p variants={contentVariants} custom={idx} className="max-w-[18rem] text-sm leading-7 text-zinc-400 md:text-[15px]">
          {service.desc}
        </m.p>

        <m.div variants={contentVariants} custom={idx} className="mt-7 flex flex-wrap items-center gap-3 text-[11px] uppercase tracking-[0.24em] text-zinc-600">
          <span>Strategy</span>
        ...
        <m.div
          ...
          custom={idx}
          className="mt-6 h-px w-full max-w-[9rem]"
          style={{ background: `linear-gradient(90deg, ${service.accent}, transparent)` }}
        />

        <m.div variants={contentVariants} custom={idx} className="mt-auto pt-10">
          <m.button
            type="button"
          ...
          >
            ...
            <span className="relative z-10 flex h-6 w-full items-center justify-center overflow-hidden leading-none">
              <span className="block whitespace-nowrap text-center transition-all duration-300 group-hover/button:-translate-y-8 group-hover/button:opacity-0">
                Know More
              </span>
              <span className="absolute block whitespace-nowrap text-center translate-y-8 opacity-0 transition-all duration-300 group-hover/button:translate-y-0 group-hover/button:opacity-100">
                Connect Now
              </span>
            </span>
          </m.button>
...
export default function Services() {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => setShowThankYou(false), 2400);
  ...
  const handleServiceSubmit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const currentService = services[openModalIdx].title;
        ...
        <div className="relative z-10 mx-auto max-w-7xl">
          ...
          >
            ...
            <p className="mx-auto mt-5 max-w-2xl text-base leading-8 text-zinc-500 md:text-lg">
              End-to-end digital marketing services for brands that want faster growth, qualified leads, and higher ROI.
            </p>
          </m.div>
          ...
          {openModalIdx !== null && (
            ...
            >
              ...
              >
                ...
                <form className="flex flex-col gap-4" onSubmit={handleServiceSubmit}>
                  <input
                    type="text"
          ...
          {showThankYou && (
            ...
            >
              <m.div
                ...
                transition={{ duration: 0.35, ease: [0.16, 1, 0.3, 1] }}
                className="relative w-full max-w-md overflow-hidden rounded-[1.6rem] border border-white/15 bg-[#0b0b12] p-6 text-center shadow-2xl"
              >
                <m.div
                  className="absolute inset-x-0 top-0 h-px"
                ...
                <div className="mx-auto mb-3 h-10 w-10 rounded-full bg-[#a78bfa22] flex items-center justify-center text-[#a78bfa] text-xl font-black">
                  ✓
                </div>
                <h4 className="text-xl font-black text-white">Thank You!</h4>
                <p className="mt-2 text-sm text-zinc-300">
                  Your request for {submittedService || "this service"} has been received.
                </p>
