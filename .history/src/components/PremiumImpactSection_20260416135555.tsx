...
function LargeCard({ stat, inView }: { stat: typeof stats[0]; inView: boolean }) {
  const count = useCountUp(stat.value, 1.2, inView);
...
function SmallCard({ stat, inView }: { stat: typeof stats[0]; inView: boolean }) {
  const count = useCountUp(stat.value, 1, inView);
  const [hov, setHov] = useState(false);
  return (
    <motion.div
      variants={cardVariants}
      ...
      <div className="relative z-20 max-w-6xl mx-auto">
        ...
        >
          <div className="md:col-span-2">
            <LargeCard stat={large} inView={hasStarted} />
          </div>
          <div className="flex flex-col gap-3 md:gap-4">
            <SmallCard stat={small[0]} inView={hasStarted} />
            <SmallCard stat={small[1]} inView={hasStarted} />
          </div>
          <SmallCard stat={small[2]} inView={hasStarted} />
          <motion.div
            variants={cardVariants}
