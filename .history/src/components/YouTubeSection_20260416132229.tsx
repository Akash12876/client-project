...
export default function YouTubeSection() {
  ...
  useEffect(() => {
    async function fetchVideos() {
      ...
      const videoItems: VideoItem[] = (data.items || []).filter(
        (item: VideoItem) => item.id.videoId
      );
      ...
      <div className="flex flex-nowrap overflow-x-auto gap-8 justify-center items-center px-4">
        {videos.map((video, idx) => {
          ...
          return (
            ...
            >
              <div className="rounded-lg overflow-hidden mb-4 h-44 bg-zinc-800 flex items-center justify-center relative">
                <Image
