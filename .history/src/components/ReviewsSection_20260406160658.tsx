...
// Bottom row uses the same reviews as the top row for identical effect
const reviewsBottom = reviewsTop;
  ...
  return (
    <section className="relative w-full py-28 px-4 md:px-0 bg-black flex flex-col items-center">
      ...
      <div className="w-full max-w-6xl flex flex-col gap-16 mt-12 items-center justify-center">
        ...
        {/* Bottom Row: right to left infinite scroll, seamless loop, never ends */}
        <div className="relative w-full flex items-center justify-start overflow-hidden min-h-[220px]">
          <div
            ref={bottomRowRef}
            style={{
              ...
              gap: "2rem",
              minWidth: "100vw",
              width: `${CARD_WIDTH * reviewsBottom.length * 2}px`,
              animation: `scrollRight ${DURATION * 2}s linear infinite`,
              animationPlayState: "running"
            }}
          >
            {[...reviewsBottom, ...reviewsBottom].map((review, idx) => (
              <div
      ...
      <style>{`
        ...
        @keyframes scrollRight {
          0% { transform: translateX(0); }
          100% { transform: translateX(${CARD_WIDTH * reviewsBottom.length}px); }
        }
      `}</style>
    </section>
  );
