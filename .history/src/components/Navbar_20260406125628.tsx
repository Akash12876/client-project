...
export default function Navbar() {
  ...
  return (
    <header className="fixed top-0 left-0 w-full z-50 flex items-center justify-between px-4 md:px-8 py-4 animate-fade-in backdrop-blur-xl bg-black/60 border-b border-white/10 shadow-lg" style={{boxShadow: "0 4px 32px 0 rgba(31,38,135,0.10)"}}>
      ...
      {/* Mobile Menu */}
      {open && (
        <div className="fixed top-0 left-0 w-full h-full bg-black/95 z-[100] flex flex-col items-center justify-center gap-8 animate-fade-in md:hidden px-4" style={{paddingTop: "4.5rem"}}>
          <button
          ...
          {navItems.map((item) => (
            <button
              key={item.label}
              onClick={() => handleNav(item.href)}
              className="text-2xl font-bold text-white py-2 px-6 rounded hover:bg-indigo-700/20 focus:outline-none focus:ring-2 focus:ring-indigo-400 w-3/4"
            >
              {item.label}
            </button>
          ))}
