    ...
    >
      ...
      <div className="flex-1 flex flex-col justify-center items-start max-w-xl w-full z-10 mb-12 lg:mb-0">
        ...
        <p className="text-white text-lg md:text-2xl font-semibold mb-8">
          We build systems that attract, convert & scale — using ads, content, and creators that drive real results.
        </p>
        <motion.button
...
Turning it into predictable revenue — that’s where we win.
          </motion.div>
          ...
          <motion.div
            className="absolute -bottom-28 left-0 z-20"
          ...
          >
            <svg width="160" height="160" viewBox="0 0 160 160" fill="none" style={{ position: 'absolute', top: 0, left: 0 }}>
              <circle cx="80" cy="80" r="70" stroke="#AD8DD8" strokeWidth="2" />
              <circle cx="80" cy="80" r="50" stroke="#A259FF" strokeWidth="2" />
              <circle cx="80" cy="80" r="30" stroke="#A259FF" strokeWidth="2" />
              <motion.circle
                cx="80" cy="80" r="16" fill="#A259FF"
                animate={{ scale: [1, 1.2, 1, 0.8, 1] }}
                transition={{ duration: 2, repeat: Infinity, repeatType: 'loop', ease: 'easeInOut' }}
                style={{ originX: 0.5, originY: 0.5 }}
              />
              <circle cx="120" cy="80" r="8" fill="#D1A3FF" />
            </svg>
          </motion.div>
        </div>
      </motion.div>
      ...
      {/* Modal Contact Form */}
        {/* Modal Contact Form */}
        {/* The modal is no longer used in WhatDefinesUs, so the code has been removed. */}
    </motion.section>
  );
}
