...
const slides = [
  ...
  // Auto-scroll logic
  const leftInterval = useReactRef(null);
  const rightInterval = useReactRef(null);
  ...
  return (
    <section className="w-full min-h-[60vh] flex flex-col items-center justify-center bg-black py-16 px-4 md:px-0">
      ...
      <div className="relative w-full max-w-6xl mx-auto flex items-center">
        ...
        {/* Cards */}
        <div className="flex gap-8 w-full justify-center overflow-hidden px-12">
          {slides.slice(index, index + visibleCount).map((slide, idx) => (
            <motion.div
              key={index + idx}
