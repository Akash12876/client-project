...
const YOUTUBE_API_KEY = process.env.NEXT_PUBLIC_YOUTUBE_API_KEY || "YOUR_YOUTUBE_API_KEY_HERE";

const staticVideos = [
  {
    id: '2XBmMsPQqI0',
    title: '', // Will be fetched
    views: '', // Will be fetched
...
export default function YouTubeSection() {
  ...
  useEffect(() => {
    // Fetch data for the first video (YouTube Shorts)
    async function fetchVideoData() {
      try {
        const res = await fetch(
          `https://www.googleapis.com/youtube/v3/videos?part=snippet,statistics&id=2XBmMsPQqI0&key=${YOUTUBE_API_KEY}`
        );
          ...
          setVideos((prev) => [
            {
              ...
              views: vid.statistics.viewCount + ' Views',
              likes: (vid.statistics.likeCount ? vid.statistics.likeCount + ' Likes' : ''),
            },
            ...prev.slice(1)
          ]);
        }
      } catch (e) {
        // fallback: do nothing, keep static
      }
    }
    fetchVideoData();
  }, []);
      ...
      >
        {videos.map((video, idx) => (
          ...
          >
            ...
            <div className="text-white text-lg font-semibold mb-2">{video.title || 'Loading...'}</div>
