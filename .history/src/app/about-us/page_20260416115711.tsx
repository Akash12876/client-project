...
const values = [
  { icon: "01", title: "Performance First", desc: "Every decision, campaign, and creative choice is anchored in real results, not vanity metrics." },
  { icon: "02", title: "Precision Targeting", desc: "We reach exactly the right audience at the right moment with data-driven strategies." },
  { icon: "03", title: "Authentic Partnerships", desc: "Brands and creators we work with become long-term allies, not just clients." },
  { icon: "04", title: "Scale Without Limits", desc: "Our systems are built to grow with you from 0 to millions, seamlessly." },
  { icon: "05", title: "Creative Intelligence", desc: "Art meets analytics. Every story we craft is rooted in insight and elevated by imagination." },
  { icon: "06", title: "Always Evolving", desc: "The digital landscape never stops changing, neither do we." },
];

const team = [
  { name: "MR PARAS SHARMA", role: "Director", color: "#a78bfa" },
  { name: "MR AKASH PANDEY", role: "CO-founder", color: "#60a5fa" },
];
...
function ValueCard({ icon, title, desc }: { icon: string; title: string; desc: string }) {
  ...
  return (
    ...
    >
      ...
      />
      <span className="text-2xl font-black text-[#a78bfa] opacity-60">{icon}</span>
...
function TeamCard({ name, role, color }: { name: string; role: string; color: string }) {
  ...
  return (
    <motion.div
      ...
      className="flex flex-col items-center gap-6 bg-gradient-to-br from-white/10 to-white/5 border-2 rounded-3xl p-12 cursor-default shadow-2xl backdrop-blur-lg relative overflow-hidden group"
    >
      <motion.div
      ...
      />
      <div
        className="w-32 h-32 rounded-full flex items-center justify-center text-5xl font-black text-white shadow-2xl relative z-10"
        style={{ background: `linear-gradient(135deg, ${color}88, ${color}44)`, border: `3px solid ${color}` }}
      >
        {name.charAt(0)}
      </div>
      <div className="text-center relative z-10">
        <div className="text-white font-black text-2xl tracking-wide">{name}</div>
        <div className="text-base mt-2 font-semibold tracking-[0.2em] uppercase" style={{ color }}>{role}</div>
      </div>
      <div className="w-16 h-1 rounded-full mt-2 relative z-10" style={{ background: `linear-gradient(90deg, transparent, ${color}, transparent)` }} />
    </motion.div>
  );
}
...
export default function AboutPage() {
  ...
  return (
    <main className="w-full bg-black text-white overflow-x-hidden">
      ...
      {/* TEAM */}
      <section id="team" className="w-full px-5 md:px-16 py-24 bg-black">
        <motion.div className="max-w-5xl mx-auto"
          initial="hidden" whileInView="show" viewport={{ once: true, amount: 0.2 }} variants={container}>
          <motion.p variants={itemAnim} className="text-[#a78bfa] text-sm font-semibold tracking-[0.3em] uppercase mb-4 text-center">The People Behind It</motion.p>
          <motion.h2 variants={itemAnim} className="text-4xl md:text-5xl font-black text-center mb-14">Meet the Team</motion.h2>
