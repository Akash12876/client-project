...
const team = [
  { name: "Arjun Sharma", role: "Founder & CEO", color: "#a78bfa" },
  { name: "Priya Mehta", role: "Head of Strategy", color: "#60a5fa" },
  { name: "Karan Joshi", role: "Creative Director", color: "#34d399" },
  { name: "Sneha Kapoor", role: "Influencer Partnerships", color: "#f472b6" },
];
...
function TeamCard({ name, role, color }: { name: string; role: string; color: string }) {
  return (
    <motion.div
      ...
      style={{ perspective: 800 }}
      className="flex flex-col items-center gap-4 bg-white/5 border border-white/10 rounded-3xl p-8 cursor-default shadow-xl backdrop-blur-sm"
    >
      <div
        className="w-20 h-20 rounded-full flex items-center justify-center text-3xl font-black text-white shadow-lg"
        style={{ background: "linear-gradient(135deg, " + color + "55, " + color + "22)", border: "2px solid " + color }}
      >
        {name.charAt(0)}
      </div>
      <div className="text-center">
        <div className="text-white font-bold text-lg">{name}</div>
        <div className="text-zinc-400 text-sm mt-1">{role}</div>
      </div>
      <div className="w-10 h-0.5 rounded-full mt-1" style={{ background: color }} />
    </motion.div>
  );
}
